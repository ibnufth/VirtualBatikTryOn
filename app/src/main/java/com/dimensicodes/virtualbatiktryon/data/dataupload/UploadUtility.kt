package com.kumastudio.cameraapp.dataupload
    ...
    fun getUrlfromWeb(filename:String,func: (m: String) -> Unit){
        Thread {
            ...
            try {
                ...
                Log.d("ricky", domainConnect.serverURL+filename)
                val response = client.newCall(request).execute()
                if (response.isSuccessful) {
                    Log.d("ricky","success, url:: "+response.request.url.toString())
                    func(response.request.url.toString())
                } else {
                    Log.d("ricky", "failed "+response.body.toString())
